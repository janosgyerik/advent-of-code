...
def main():
    with open('tmp/input.txt') as fh:
        ...
        prompt = [ord(c) for c  in 'Command?\n']
        while not m.halted:
            m.execute()
            if m.outputs[-len(prompt):] == prompt:
                print(''.join([chr(x) for x in m.outputs]))
                m.outputs.clear()
                ...
                m.inputs.extend(ord(c) for c in command + "\n")
